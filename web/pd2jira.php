...
if ($messages) foreach ($messages->messages as $webhook) {
  ...
  switch ($webhook_type) {
    case "incident.trigger":
      ...
      //Changing to: “An escalated incident was reported from PagerDuty. [insert PD summary defaults here]
      $data = array('fields'=>array('project'=>array('key'=>"$jira_project"),'summary'=>"$summary",'description'=>"An escalated incident was reported from PagerDuty.  {$trigger_summary_data}. Please go to $ticket_url to view it.", 'issuetype'=>array('name'=>"$jira_issue_type")));
